...
public class PaymentProcessor {
    ...
    public TransactionResult processPayment(Transaction transaction) {
        ...
        return new TransactionResult(null, TransactionStatus.Failed, 0, "Card not supported.",
            transaction, 0.0, 0.0, 0.0);
    }

    private PaymentProvider determineProvider(Transaction transaction) {
        Map<String, PaymentProvider> providersByName = providers.stream().collect(Collectors.toMap(
        ...
        for (PaymentProvider paymentProvider : providers) {
            try {
                providerCharges.putIfAbsent(paymentProvider.getName(),
                    paymentProvider.calculateTransactionFee(transaction));
            } catch (UnsupportedTransactionException e) {
                logger.info("Transaction not supported for this {} " ,paymentProvider.getName());
            }
        }
